...
Lectio Sync is an open-source, local-first desktop browser extension that keeps a student's Lectio timetable in a dedicated calendar. It supports Chrome and Brave through Chrome Identity, Firefox through a PKCE-protected installed-app OAuth flow, and Safari through a small EventKit bridge generated as an Xcode app extension.
...
Requirements: Node.js 20 or later. Safari additionally requires macOS and Xcode.
...
| Brave desktop | Supported; Google login for extensions may need enabling in Brave settings | Chrome |
...
Load `dist/chrome` from `chrome://extensions` using **Developer mode → Load unpacked**. Calendar authorization needs a Chrome Extension OAuth client bound to the extension ID:

1. Create a Google Cloud project and enable the Google Calendar API.
2. Configure its OAuth consent screen.
3. Create a **Chrome Extension** OAuth client using the unpacked extension ID or Chrome Web Store item ID.
4. Build or package with the client ID:

```sh
GOOGLE_OAUTH_CLIENT_ID="your-id.apps.googleusercontent.com" npm run build:chrome
GOOGLE_OAUTH_CLIENT_ID="your-id.apps.googleusercontent.com" npm run package:chrome
```

The release ZIP is written to `artifacts/lectio-sync-chrome.zip`, with `manifest.json` at the archive root. Packaging deliberately fails if the OAuth ID is still the placeholder. A client secret is neither needed nor allowed in the extension.
...
GOOGLE_FIREFOX_OAUTH_CLIENT_ID="your-id.apps.googleusercontent.com" npm run package:firefox
```

The signed add-on must keep the `browser_specific_settings.gecko.id` value stable because Firefox derives its OAuth redirect identity from the add-on ID. The release ZIP is written to `artifacts/lectio-sync-firefox.zip`. No OAuth client secret is bundled.
